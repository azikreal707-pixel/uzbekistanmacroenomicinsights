...
# DASHBOARD HEADER
        ...
        [
            html.Div(
                [
                    ...
                    dcc.Graph(id="line-chart", style={"height": "70vh", "width": "100%",}),
                    html.Div(
                        [
            ...
            html.Div(
                [
                    ...
                    dcc.Graph(id="region-area-chart", style={"height": "70vh", "width": "100%"}),
                    html.Div(
                        [
                            html.Div(
                                [
    ...
    # === BUDGET SECTOR DISTRIBUTION ===
    html.H2("Budget Income and Expenditure by Sector", className="section-title"),
    html.Div([
    html.Div([
        html.Label("Select Year:", className="dropdown-label1"),
        dcc.Dropdown(
    ...
    html.Div([
        html.Div([
            dcc.Graph(id="donut-income", style={"height": "500px", "width": "100%"}),
        ], style={"flex": "1", "marginRight": "10px"}),

        html.Div([
            dcc.Graph(id="donut-expense", style={"height": "500px", "width": "100%"}),
    ...
    html.Hr(style={"border": "1px solid #ccc", "marginTop": "20px"}),
...
@app.callback(
    Output("donut-income", "figure"),
    Output("donut-expense", "figure"),
    Input("donut-year", "value"),
)
def update_donut_charts(selected_year):
    dyear = df[df["Year"] == selected_year]

    # --- Income
    inc = dyear[(dyear["Metric"] == METRICS["income"]) & (dyear["Sector"].notna()) & (dyear["Sector"] != "")]
    inc_agg = inc.groupby("Sector", as_index=False)["Value"].sum()
    fig_income = px.pie(
        inc_agg,
        names="Sector",
        values="Value",
        hole=0.45,
        title=f"Budget Income by Sector — {selected_year}",
    ...
    # --- Expense 
    exp = dyear[(dyear["Metric"] == METRICS["expense"]) & (dyear["Sector"].notna()) & (dyear["Sector"] != "")]
    exp_agg = exp.groupby("Sector", as_index=False)["Value"].sum()
    fig_expense = px.pie(
        exp_agg,
        names="Sector",
        values="Value",
        hole=0.45,
        title=f"Budget Expenditure by Sector — {selected_year}",
